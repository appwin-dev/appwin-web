...
export class SupportRealtime {
  ...
  private stopWatchingIdentity: (() => void) | null = null
  ...
  async connect(): Promise<void> {
    this.closed = false
    // The token's topics belong to one customer: after identify or logout the
    // socket would keep delivering the previous one's events.
    this.stopWatchingIdentity ??= this.options.session.onIdentityChange(() => this.reopen())
    await this.open()
  }
  ...
  disconnect(): void {
    this.closed = true
    this.stopWatchingIdentity?.()
    this.stopWatchingIdentity = null
    this.clearTimers()
  ...
  private reopen(): void {
    ...
    this.socket = null
    if (previous) {
      // Detached first, so its close does not schedule a reconnect of its own.
