...
async function framesSentOnOpen(token: string): Promise<string[]> {
  ...
  try {
    const realtime = new SupportRealtime({
      session: {
        fetch: async () => ({ token, expiresIn: 60 }),
        onIdentityChange: () => () => {},
...
describe('identity change', () => {
  it('reconnects with a token for the new visitor and drops the old socket', async () => {
    const original = globalThis.WebSocket
    ;(globalThis as { WebSocket: unknown }).WebSocket = FakeSocket
    const listeners: Array<() => void> = []
    const tokens = [tokenWithTopics(['sdk:customer:before']), tokenWithTopics(['sdk:customer:after'])]

    try {
      const realtime = new SupportRealtime({
        session: {
          fetch: async () => ({ token: tokens.shift(), expiresIn: 60 }),
      ...
      const after = FakeSocket.last!
      after.onopen?.()

      assert.equal(before.closedWith, 1000)
      assert.notEqual(after, before)
      assert.deepEqual(after.sent, ['{"a":"sub","topic":"sdk:customer:after"}'])
