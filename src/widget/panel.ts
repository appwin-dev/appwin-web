 ...
 * four: open, close, identify, logout (plus attribute updates).
 *
...
function readParams(): PanelParams | null {
  ...
  return {
    ...
    // `data-user-id` is what a cached 0.7 loader writes (see protocol.ts).
    externalId: script.getAttribute('data-external-id') ?? script.getAttribute('data-user-id'),
  }
}
...
function boot(params: PanelParams): void {
  ...
  appwin.support.onVisitorChange(() => void store.visitorChanged())
  ...
  window.addEventListener('message', (event: MessageEvent) => {
    ...
    switch (message.type) {
      ...
      case 'close':
        break
      // Failures stay in here: the host posted a message and cannot be handed
      // a rejection, and the session retries on the next call anyway.
      case 'identify':
        appwin.identify(message.externalId, message.attributes).catch(() => {})
        break
      case 'updateUser':
        appwin.updateUser(message.attributes).catch(() => {})
        break
      case 'logout':
        void appwin.logout()
        break
