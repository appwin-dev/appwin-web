...
export interface AppwinWidget {
  ...
  close(): void
  toggle(): void
  /** Attaches this browser to one of the studio's own users, with optional attributes. */
  identify(externalId: string, attributes?: AppwinUserAttributes): void
  /** Writes what the studio knows about the current user; omitted fields are left as they are. */
  updateUser(attributes: AppwinUserAttributes): void
  /** For when the studio's user signs out: the widget continues as a new anonymous visitor. */
  logout(): void
  /** @deprecated Use `logout()`. Kept because 0.7 snippets on the mutable `v1/` URL call it. */
  reset(): void
}
...
function readConfig(): LoaderConfig | null {
  ...
  return {
    ...
    // The 0.7 attribute keeps working: snippets load the mutable `v1/` URL and
    // were pasted once, with no upgrade step to rename anything.
    externalId:
      script.getAttribute('data-appwin-external-id') ?? script.getAttribute('data-appwin-user-id'),
  }
}
 ...
 * it never lands in a URL, a referrer or a server log, which is what an `externalId`
...
function panelDocument(config: LoaderConfig): string {
  const data = [
    ...
    ['data-external-id', config.externalId],
  ]
...
function mount(config: LoaderConfig): AppwinWidget {
  ...
  // Nothing reaches the panel before its `ready`: see `createMailbox`.
  const mailbox = createMailbox<ReturnType<typeof hostMessage>>((message) => {
    panel.contentWindow?.postMessage(message, panelOrigin)
  })
  const post = mailbox.post
  ...
  return {
    ...
    identify: (externalId: string, attributes?: AppwinUserAttributes) =>
      post(hostMessage({ type: 'identify', externalId, ...(attributes ? { attributes } : {}) })),
    updateUser: (attributes: AppwinUserAttributes) => post(hostMessage({ type: 'updateUser', attributes })),
    logout: () => post(hostMessage({ type: 'logout' })),
    reset: () => post(hostMessage({ type: 'logout' })),
  }
}
