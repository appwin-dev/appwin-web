...
export type HostMessage =
  | { v: 1; from: typeof HOST; type: 'open' }
  | { v: 1; from: typeof HOST; type: 'close' }
  | { v: 1; from: typeof HOST; type: 'identify'; userId: string }
  | { v: 1; from: typeof HOST; type: 'reset' }

/** Panel to host page. */
export type PanelMessage =
  /** Config is in: the panel can be shown, and the launcher knows its colours. */
  | { v: 1; from: typeof PANEL; type: 'ready'; primary: string; primaryForeground: string }
  | { v: 1; from: typeof PANEL; type: 'unread'; count: number }
  /** The panel's own close button, so the launcher returns to its idle state. */
  | { v: 1; from: typeof PANEL; type: 'close' }
...
function envelope(raw: unknown, from: string): Record<string, unknown> | null {
  ...
  if (message.v !== WIDGET_PROTOCOL_VERSION) return null
  return message
}
...
export function readHostMessage(raw: unknown): HostMessage | null {
  ...
  switch (message.type) {
    ...
    case 'identify':
      ...
      return hostMessage({ type: 'identify', userId: message.userId })
    default:
      return null
  }
}
