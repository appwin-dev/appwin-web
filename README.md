...
| `data-appwin-external-id` | your own id for a signed-in user; absent means an anonymous visitor |
...
Appwin.close()
Appwin.toggle()
Appwin.identify('user-123', { email: 'ada@example.com' })  // when your own user signs in
Appwin.updateUser({ plan: 'pro' })                         // omitted fields are left as they are
...
// Identity: sign in, enrich, sign out. Each one also resets what the messenger shows.
