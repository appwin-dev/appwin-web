...
declare class Session {
     ...
     */
    identify(externalId: string, traits?: AppwinUserTraits): Promise<void>;
    /** Writes traits on the current user, anonymous or identified. */
    updateUser(traits: AppwinUserTraits): Promise<void>;
    /**
...
declare class SupportClient {
    ...
    /**
     * Called when the visitor behind this client changes: `identify` with a new
     * user, or `logout`. Everything read before belongs to the previous visitor,
     * so an interface drops it and refetches. Returns the unsubscribe function.
     */
    onVisitorChange(listener: () => void): () => void;
...
declare class SupportRealtime {
    ...
    private retryTimer;
    private closed;
    private stopWatchingIdentity;
    /** Bumped by `reopen`, so a token minted for the previous identity is dropped. */
    private generation;
    private readonly options;
...
interface Appwin {
     ...
     * no new session. `traits`, when given, are written as by `updateUser`.
     *
